...
function SignIn({open,setOpen}) {
  ...
  const auth = useSelector((globalState) => globalState.authReducer);
  // console.log(auth);
    ...
    <div>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">            
            <div className="py-2">
                ...
                <input type="email" onChange={handleChange} name="review-title" className='border w-full py-2 px-2 rounded focus:outline-none ' placeholder='e.g, abc@gmail.com' id="email" />
            </div>
            <div className='py-3'>
                <label htmlFor="password" className='text-lg'>Enter Your Password</label>
                <input type="password" onChange={handleChange}  name="password" className='border w-full py-2 px-2 rounded focus:outline-none' placeholder='e.g, ••••••••' id="password" />
            </div>
        ...
        <DialogActions >
            ...
            <button  onClick={handleSubmit} className='border px-3 py-1 rounded text-white duration-300 bg-zomato-400'>
                Login
            </button>
          </div>
        </DialogActions>
